...
public class SqlExecutorInterceptor implements Interceptor {
    ...
    private static String PATTERN_SQL1 = "";
    ...
    public Object intercept(Invocation invocation) throws Throwable {
        ...
        // 这个是获取参数信息，如果没有author这个参数名称/这个里面的值为null/为管理员权限，则不进行sql权限拦截和修改
        ...
        for (Method method : classType.getDeclaredMethods()) {
            ...
            if (method.isAnnotationPresent(AuthorControl.class) && (mName.equalsIgnoreCase(method.getName()) || mName.equalsIgnoreCase(method.getName() + "_count"))) {
                AuthorControl authorControl = method.getAnnotation(AuthorControl.class);
                String sql = boundSql.getSql();
                log.info("拦截器StatementInterceptor拦截前的sql语句为:" + (StringUtils.isBlank(sql) ? "" : sql.replaceAll("\n", " ")));
