...
public class FileUtil {
	...
	public static void delEmptyDirNoFin(File path,boolean isDelSelf){
		...
		if(path.exists() && path.isDirectory()) {
			...
			if(children != null && children.length>0) {
				for(File child : children) {
					if(child.toString().contains("finished")){
						continue;
					}
					delEmptyDir(child);
				}
					...
					if (isContion(fileTypes,files[i].getAbsolutePath())) {//文件存在
						...
						if(isHasChar(files[i].getAbsolutePath().toLowerCase(),str_filter)){
							...
							if(flag){
								...
								log.info("过滤文件" + files[i].getAbsolutePath()+"删除失败");
							}
	...
	public static String moveFile(String fileName,String destFolder,String fileType) {
		...
		try {
			...
			//移到目标文件夹时，需要进行中间名称
			String tempSrcFileName=srcFile.getAbsolutePath().substring(0,srcFile.getAbsolutePath().indexOf("."))+".ing";
			...
			boolean flag1 = srcFile.renameTo(tempSrcFile);
			if(flag1){
				log.info("文件改名成功");
			}
			FileUtils.moveFileToDirectory(tempSrcFile, destDir, true);

			//将ing名字更换回来
			String destTempFileName=destDir+File.separator+tempSrcFile.getName();
			newFilePath=destDir+File.separator+srcFile.getName();

			File destTempFile=new File(destTempFileName);
			File newFilePathFile=new File(newFilePath);
			Boolean flag2 = destTempFile.renameTo(newFilePathFile);
			if(flag2){
				log.info("文件改名成功");
			}
		} catch (IOException e) {
	...
	public static boolean moveFile(List<String> fileNameList,String destFolder,String fileType){
		for (String filePath : fileNameList) {
			File f=new File(filePath);
			if(f.exists()){
				FileUtil.moveFile(filePath, destFolder,fileType);
			}
		}
		return true;
	}
...
//数据上传下发专用-------------------------------------------------
			...
			for (int i = 0; i < files.length; i++) {
				...
				if (f.isDirectory() == true){//目录
					...
					/*if(fileType==null || fileType.trim().length()==0){
						...
						String[] fileTypes=fileType.toLowerCase().split(",");
						if (isContion(fileTypes,f.getAbsolutePath())) {
							String filePath = f.getAbsolutePath().replace(File.separator.charAt(0), '/');
							fileList.add(filePath);
						}
					}*/
	...
	private static boolean checkFileByName(File f,String fileNameExp){
		...
		if(fileNameExp==null || fileNameExp.trim().length()==0 || fileNameExp.equals("%")){
			flag=true;
		}else {
