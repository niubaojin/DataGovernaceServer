...
import com.synway.datarelation.constant.CommonErrorCode;
 ...
 * configuration.getString("runnable.container.job.class"); <br>
 * <p/>
 * <br>
 * 设置多级List <br>
...
public class Configuration {
	...
	public static Configuration newDqcDefault() {
		Configuration configuration = newDefault();
		configuration.set("job.setting.addID","false");
		configuration.set("job.setting.report","false");
		configuration.set("job.setting.speed.channel",Runtime.getRuntime().availableProcessors());
	...
	public static Configuration from(String json) {
        json = StringUtilNew.replaceVariable(json);
		checkJSON(json);
