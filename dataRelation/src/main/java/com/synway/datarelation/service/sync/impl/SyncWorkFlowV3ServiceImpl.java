 ...
 */
@Service
public class SyncWorkFlowV3ServiceImpl implements SyncWorkFlowV3Service {
    private static Logger logger = LoggerFactory.getLogger(SyncWorkFlowV3ServiceImpl.class);
    @Autowired
    TaskPageQueryDao taskPageQueryDao;
//    @Autowired
//    @Qualifier(value = "TaskDefinitionInstanceServiceImpl")
//    TaskDefinitionInstanceService taskDefinitionInstanceServiceImpl;
    @Autowired
    ...
    @Override
    public DataSynchronizationReportPageReturn getDataSynchronizationReport(DataSynchronizationReportPageParams queryParams) throws Exception {
...
//            taskDefinitionInstanceServiceImpl.getTaskRunLog(taskRunQueryParameters,true);
     ...
     */
    @Override
    public Map<String,Object> queryHistoryTaskLogService(HistoryTaskQueryParams historyTaskQueryParams)
        throws Exception{
...
//            taskQueryParameters.setOrderBy("createTime%desc");
            // 20201202  使用方式发生变化
            DataWorkV3Service dataWorkV3ClientService = new DataWorkV3Service();
            dataWorkV3ClientService.getTask(taskQueryParameters , true);
//            taskDefinitionInstanceServiceImpl.getTask(taskQueryParameters , true);
            Boolean flag = true;
     ...
     */
    @Override
    public void exportDataTableService(DataSynchronizationReportPageParams queryParams, HttpServletResponse response, int version) {
        try{
            //文件名称
            ...
            queryParams.setTaskOpenDate(queryParams.getTaskOpenDate()+" 00:00:00");
            queryParams.setTaskEndDate(queryParams.getTaskEndDate()+" 23:59:59");
            List<DataReportPageReturn> lists = null;
            if(version == 3){
                lists =taskPageQueryDao.getDataSynchronizationReportDao(queryParams);
            }else{
                lists = taskQueryV2Dao.getDataSynchronizationReportTwo(queryParams);
            }
            for(DataReportPageReturn dataReportPageReturn:lists){
    ...
    @Override
    public void exportHistoryTaskTable(HistoryTaskQueryParams historyTaskQueryParams, HttpServletResponse response,int version) {
        try{
            //文件名称
            ...
            List<DataReportPageReturn> lists = null;
            if(version == 3){
                lists = taskPageQueryDao.queryHistoryTaskLogDao(historyTaskQueryParams);
            }else{
                lists = taskPageQueryDao.queryHistoryTaskLogTwoDao(historyTaskQueryParams);
            }
            //响应
            ServletOutputStream out=response.getOutputStream();
            response.setContentType("application/json;charset=UTF-8");
...
//-----------------------------------------------------------------------------------------------------------------------------
    //      以下为datawork 第二版本的页面查询程序

    /**
     * 页面查询的搜索按钮   dataWork 第二版本
     * @param queryParams
     * @return
     * @throws Exception
     */
    @Override
    public DataSynchronizationReportPageReturn getDataSynchronizationReportTwo(DataSynchronizationReportPageParams queryParams) throws Exception {
        DataSynchronizationReportPageReturn dataSynchronizationReportPageReturn = new DataSynchronizationReportPageReturn();
        //        分页插件的使用
        Page page=  PageHelper.startPage(queryParams.getPageNum(),queryParams.getPageSize());
     ...
     */
    @Override
    public String queryDataSynchronizationTwoLog(String taskId, String id) throws Exception {
        String logMessage = taskPageQueryDao.queryDataSynchronizationLogDao(taskId,id);
        if(StringUtils.isEmpty(logMessage)){
    ...
    @Override
    public Map<String, Object> queryHistoryTaskTwoLogService(HistoryTaskQueryParams historyTaskQueryParams) throws Exception {
        //  如果查询flag为true，表示需要从 api 中重新查询数据
        //  如果为false 表示直接从数据库中获取
