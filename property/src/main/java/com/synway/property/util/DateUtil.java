...
private static Logger log = LoggerFactory.getLogger(DateUtil.class);
	...
	public final static String	DEFAULT_PATTERN_DATE1					= "yyyy/MM/dd";
	...
	public static String formatDateSimple(Date date) {
		return formatDateTime(date, DEFAULT_PATTERN_DATE_SIMPLE);
	}
	...
	public static String addDayStr(Date oldDate, int days) {
		Calendar cal = Calendar.getInstance();
		cal.setTime(oldDate);
		cal.add(Calendar.DAY_OF_YEAR, days);
		return formatDateSimple(cal.getTime());
	}

	// 返回 "yyyy/MM/dd" 格式字符串
	public static String addDayStr1(Date oldDate, int days) {
		Calendar cal = Calendar.getInstance();
		cal.setTime(oldDate);
		cal.add(Calendar.DAY_OF_YEAR, days);
		return formatDateTime(cal.getTime(), DEFAULT_PATTERN_DATE1);
	}
	...
	public static Date getDate(String datetimeStr,String pattern){
		Date date=null;
		if(datetimeStr==null || datetimeStr==""){//如果前端没有设置日期，则默认为今天
			date=new Date();
		}else{
			try {
