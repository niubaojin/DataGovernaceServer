...
public interface DataStorageMonitorDao {
     ...
     * @return
     */
    List<DataResourceTable> getAllDataBaseCount();
     ...
     * 获取数据资产中最右上角的页面数据
     */
//    @AuthorControl(tableNames ={"DP_TABLE_ORGANIZATION_ASSETS","detect_task_manage"},columnNames = {"resourceid","taskID"})
    ...
    Integer delDataStorageTable(@Param("item") NeedAddRealTimeTable needAddRealTimeTable, @Param("type") String type);
     ...
     * @return
     */
//    @AuthorControl(tableNames ={"DP_DATA_STORAGE_ADD_TABLE"},columnNames = {"table_id"})
...
//    @AuthorControl(tableNames ={"DP_DATA_STORAGE_ADD_TABLE"},columnNames = {"table_id"})
    ...
    @AuthorControl(tableNames ={"DP_TABLE_ORGANIZATION_ASSETS"},columnNames = {"resourceid"})
    List<PageSelectOneValue> getPrimaryClassifyData(@Param("mainClassify") String mainClassify,@Param("daysAgo") int daysAgo, @Param("sjzzflCodeId") String sjzzflCodeId);
...
//    List<String> getLabelsByName(@Param("list") List<ReceiveTag> labelTagsList);
    ...
    void delTableOrganizationShowField(@Param("tableOrganizationShowFields") String tableOrganizationShowFields, @Param("userName") String userName);
