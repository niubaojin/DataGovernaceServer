...
public class LiquibaseConfig {
    ...
    public SpringLiquibase liquibase(DataSource dataSource){
        ...
        String dbType = Objects.requireNonNull(env.getProperty("database.type")).toLowerCase();
        String path=String.format("sql%s%s%schangelog.xml",File.separator,dbType,File.separator);

        //必须使用绝对路径，不然sql分开打包不能识别
        String changeLogXml = String.format("classpath:%s", path);
         ...
         * 应用模块根据模块表前缀自行定义
         */
        liquibase.setDatabaseChangeLogTable("DAC_KX_SCHEMA_CHANGE_LOG");
        //多用户场景使用
        liquibase.setDatabaseChangeLogLockTable("DAC_KX_SCHEMA_CHANGE_LOG_LOCK");
        return  liquibase;
    }
