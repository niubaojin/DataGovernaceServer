...
public class FileUtil {
	...
	public static void delEmptyDir(File path){
		if(path.exists() && path.isDirectory()) {
			...
			if(children != null && children.length>0) {
				...
				boolean deleteFlag = path.delete();
				log.info(deleteFlag ? "文件删除成功" : "文件删除失败");
			}else {
				boolean deleteFlag = path.delete();
				log.info(deleteFlag ? "文件删除成功" : "文件删除失败");
			}
	...
	public static void writeToFile(String filepath,String msg, boolean append){
		...
		if(!file.exists()){
			try {
				...
				log.info(createFlag ? "文件新建成功" : "文件新建失败");
			} catch (IOException e) {
				log.error(e.toString());
			}
		}
		...
		try {
			for (int i = 0; i < files.length; i++) {
				...
				if (files[i].isDirectory()){
					...
					//需要排除特定目录
					if (excludeSubFolders != null && excludeSubFolders.size() > 0) {
						//当前目录不在排除列表内
						if (!excludeSubFolders.contains(files[i].getName())) {
					...
					if(fileType==null || fileType.trim().length()==0){
						...
						if (isContion(fileTypes,files[i].getAbsolutePath())) {
							//BissProxyLog.log(log, "info", "得到的文件路径:   " + files[i].getAbsolutePath(),null, false, true);
							String filePath = files[i].getAbsolutePath().replace(File.separator.charAt(0), '/');
							fileList.add(filePath);
						}
					}
	 ...
	 * @param strFilter 文件名中被过滤的内容
	 * @return
	 * @return:		 ArrayList<String>
	 * @throws
	 */
	public static ArrayList<String> searchFileBydel(String folderPath,String fileType,boolean cascade,List<String> excludeSubFolders,String strFilter){
		ArrayList<String> fileList=new ArrayList<String>();
		File[] files =null;
		File myDir = new File(folderPath);
		//列出所有文件夹和文件
			...
			for (int i = 0; i < files.length; i++) {
				...
				if (files[i].isDirectory()){
					...
					if (excludeSubFolders != null && excludeSubFolders.size() > 0) {
						//当前目录不在排除列表内
						if (!excludeSubFolders.contains(files[i].getName())) {
							ArrayList<String> subFileList = searchFileBydel(files[i].getAbsolutePath(), fileType, cascade, excludeSubFolders, strFilter);
							if (subFileList != null && !subFileList.isEmpty()) {
								fileList.addAll(subFileList);
							} else {
								log.info("空文件夹" + files[i].getAbsolutePath() + "删除成功" + files[i].delete());
					...
					//文件存在
					if (isContion(fileTypes,files[i].getAbsolutePath())) {
						if(isHasChar(files[i].getAbsolutePath().toLowerCase(),strFilter)){
							boolean deleteFlag = files[i].delete();
							log.info("过滤文件"+files[i].getAbsolutePath()+ (deleteFlag ?"删除成功" : "文件删除失败"));
						}else{
	...
	public static String moveFile(String fileName,String destFolder,String fileType) {
		...
		try {
			...
			if(destFile.exists()){
				boolean deleteFlag = destFile.delete();
				log.info(deleteFlag ? "文件删除成功" : "文件删除失败");
			}
	 ...
	 * @return
	 */
	private static boolean isHasChar(String s,String strFilter){
		boolean flag=false;
		String charFilter=strFilter;
	...
	*@return
	...
	public static String renameFileSuffix(String srcFilePath,String suffix){
		...
		try {
			if(!suffix.startsWith(Constants.CHARACTER_POINT)){
				suffix = Constants.CHARACTER_POINT + suffix;
			}
			newFilePath=srcFilePath.substring(0,srcFilePath.indexOf(Constants.CHARACTER_POINT))+suffix;
...
//数据上传下发专用-------------------------------------------------
	...
	public static int moveFile(File oldFile, String destFolder,
		...
		if (oldFile.exists() && oldFile.isFile()) {
			...
			if(oldFolder.equals(oldFileParent+File.separator)) {
				childFolder = oldFileParent.substring(oldFolder.length()-1,oldFileParent.length());
			}else {
				childFolder = oldFileParent.substring(oldFolder.length(),oldFileParent.length());
			}
			...
			boolean deleteFlag = newFile.delete();
			log.info(deleteFlag ? "文件删除成功" : "文件删除失败");
	...
	public static int copyFile(File srcFile, String destFolder,
		...
		if (srcFile.exists() && srcFile.isFile()) {
			...
			if(srcFolder.equals(oldFileParent+File.separator)) {
				childFolder = oldFileParent.substring(srcFolder.length()-1,oldFileParent.length());
			}else {
				childFolder = oldFileParent.substring(srcFolder.length(),oldFileParent.length());
			}
			if(SysUtil.isNullStr(childFolder)) {
				try{
					newFilePath = generatePath(destFolder,null);
		...
		try {
			for (int i = 0; i < files.length; i++) {
				...
				if (f.isDirectory()){
					...
					if (excludeSubFolders != null && excludeSubFolders.size() > 0) {
						//当前目录不在排除列表内
						if (!excludeSubFolders.contains(f.getName())) {
	...
	private static boolean checkFileByName(File f,String fileNameExp){
		...
		if(fileNameExp==null || fileNameExp.trim().length()==0 || StringUtils.equals(Constants.CHARACTER_PERCENT, fileNameExp)){
			flag=true;
		}else {
	...
	public static int getLineNum(File file,boolean isNeedFirstLine){
		...
		try {
			...
			while((trim=br.readLine())!=null&&!trim.trim().equals("")){
				count++;
			}
		} catch (Exception e) {
			return -1;
	...
	public static void main(String[] args) {
		List<String> list=readFileContent("D:\\BissData\\oracle批量抽取测试\\20180126093823456_1.txt", null);
