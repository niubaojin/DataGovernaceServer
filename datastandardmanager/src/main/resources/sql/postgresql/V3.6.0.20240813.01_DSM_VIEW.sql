...
FROM (SELECT *
      ...
      WHERE CODEID NOT IN
            (SELECT CODEID
             FROM synlte.FIELDCODE
             WHERE DELETED != '0'
               and PARCODEID like 'JZCODEGASJZZFL%')) v
where v.CODEID like 'JZCODEGASJZZFL%'
UNION ALL
...
FROM (SELECT *
      ...
      WHERE CODEID NOT IN
            (SELECT CODEID
             FROM synlte.FIELDCODE
             WHERE DELETED != '0'
               and PARCODEID like 'GACODE000404%')) v
where v.CODEID like 'GACODE000404%';
...
objectmemo as MEMO,
-- 数据的插入时间
case when MODDATE is null then CURRENT_DATE else MODDATE end as INSERT_DATE,
-- 表名
...
from(
   ...
   ) subquery where tableid is not null
) tt;
