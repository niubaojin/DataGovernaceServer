...
FROM
    synlte.FIELDCODE
WHERE
    CODEID ='JZCODEGASJZZFL' UNION ALL
SELECT
    'JZCODEGASJZZFL' root,
    CODEVALID CODEID,
    VALVALUE CODENAME,
...
FROM
    (
        ...
        FROM
            synlte.FIELDCODEVAL
        WHERE
            CODEID NOT IN ( SELECT CODEID FROM synlte.FIELDCODE WHERE DELETED != '0' START WITH PARCODEID ='JZCODEGASJZZFL' CONNECT BY PRIOR CODEID = PARCODEID )) v START WITH v.CODEID ='JZCODEGASJZZFL' CONNECT BY PRIOR v.CODEVALID = v.CODEID UNION ALL
SELECT
    'GACODE000404' root,
    CODEID,
    CODENAME,
    CASE
        WHEN CODEID = 'GACODE000404' THEN
            '数据来源分类' ELSE CODETEXT
        END AS CODETEXT,
    PARCODEID
FROM
    synlte.FIELDCODE
WHERE
    CODEID ='GACODE000404' UNION ALL
SELECT
    'GACODE000404' root,
    CODEVALID CODEID,
    VALVALUE CODENAME,
...
(
    ...
    FROM
        synlte.FIELDCODEVAL
    WHERE
        CODEID NOT IN ( SELECT CODEID FROM synlte.FIELDCODE WHERE DELETED != '0' START WITH PARCODEID='GACODE000404' CONNECT BY PRIOR CODEID = PARCODEID )) v START WITH v.CODEID ='GACODE000404' CONNECT BY PRIOR v.CODEVALID = v.CODEID;
