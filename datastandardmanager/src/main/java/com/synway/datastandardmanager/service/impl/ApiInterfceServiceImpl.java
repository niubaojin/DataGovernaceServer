...
public class ApiInterfceServiceImpl implements ApiInterfceService {
    ...
    @Override
    public List<ValueLabelVO> getStandardOutTableIdBySourceIdService(String sourceId, String sourceCode, String sourceFirmCode) {
        List<ValueLabelVO> valueLabelVOS = new ArrayList<>();
        try {
            ...
            valueLabelVOS = standardizeInputObjectRelateMapper.getStandardOutTableIdBySourceIdDao(sourceId, sourceCode, String.valueOf(sourceFirmNum));
        } catch (Exception e) {
            log.error(">>>>>>查询数据报错：", e);
        }
        return valueLabelVOS;
    }
    ...
    public List<Map<String, String>> getStandardTableBySourceId(String sourceId) {
        ...
        try {
            ...
            List<ValueLabelVO> valueLabelVOS = standardizeInputObjectRelateMapper.getStandardOutTableIdBySourceIdDao(sourceId, "", "");
            for (ValueLabelVO valueLabelVO : valueLabelVOS) {
                Map<String, String> map = new HashMap<>(1);
                map.put("tableId", valueLabelVO.getValue());
                jsonObjects.add(map);
            }
