 ...
 */
@Slf4j
public class CreateTableSqlServiceImplCk implements CreateTableSqlService {
    ...
    public String getAddColumnSql(CreateTableInfoDTO data) {
        log.info(">>>>>>开始拼接clickhouse新增字段的相关操作");
    ...
    @Override
    public String addColumnByData(CreateTableInfoDTO data) throws Exception {
        String sqlStr = this.getAddColumnSql(data);
        log.info("在clickhouse中生成的sql语句为：" + sqlStr);
    ...
    public String getCreateSql(BuildTableInfoVO buildTableInfoVo) {
        ...
        String tableNameLocal = tableName.toLowerCase() + "_local";
        String tableNameCluster = tableName.toLowerCase();
        if (!buildTableInfoVo.getIsCreatTableTool()) {
            tableNameLocal = projectName.toLowerCase() + "." + tableName.toLowerCase() + "_local";
            tableNameCluster = projectName.toLowerCase() + "." + tableName.toLowerCase();
        }
        ...
        if (buildTableInfoVo.getIsCreatTableTool()) {
            stringBuilder.append("jz, ").append(tableName.toLowerCase()).append("_local").append(", rand());");
        } else {
            stringBuilder.append(projectName.toLowerCase()).append(", ").append(tableName.toLowerCase()).append("_local")
                    .append(", rand());");
        }
    ...
    public String createTableByPage(BuildTableInfoVO buildTableInfoVo) {
        String message;
        try {
            log.info(">>>>>>clickhouse建表的相关信息为：" + JSONObject.toJSONString(buildTableInfoVo));
            RestTemplateHandle restTemplateHandle = SpringBeanUtil.getBean(RestTemplateHandle.class);
            ...
            log.info(">>>>>>生成的sql语句为：" + createSql.replaceAll("\\n", "  "));
            String sqlLocal = createSql.split(";")[0];
            String sqlCluster = createSql.split(";")[1];
            // 本地表sql
            CreateTableDataVO createTableDataLocal = new CreateTableDataVO();
            ...
            // 集群表sql
            CreateTableDataVO createTableDataCluster = new CreateTableDataVO();
    ...
    public String createTableBeforeCheck(BuildTableInfoVO buildTableInfoVo) {
        ...
        // 检查必填参数是否存在
        if (buildTableInfoVo.getCkOrderByColumn() == null || buildTableInfoVo.getCkOrderByColumn().isEmpty()) {
            throw SystemException.asSystemException(ErrorCodeEnum.UNION_ERROR, "排序字段是必填的");
        }
        ...
        if (buildTableInfoVo.getColumnData() == null || buildTableInfoVo.getColumnData().isEmpty()) {
            throw SystemException.asSystemException(ErrorCodeEnum.UNION_ERROR, "建表字段不能为空");
        }
        if (buildTableInfoVo.getLifeCycle() == null) {
            throw SystemException.asSystemException(ErrorCodeEnum.UNION_ERROR, " 生命周期不能设置为0");
        }
        if (buildTableInfoVo.getLifeCycle() != null) {
            boolean flag = buildTableInfoVo.getColumnData().stream().anyMatch(d -> StringUtils.equalsIgnoreCase(d.getColumnName(), "capture_time"));
            if (!flag) {
                throw SystemException.asSystemException(ErrorCodeEnum.UNION_ERROR, " 当表设置了生命周期时，capture_time字段必须存在");
            }
        }
        ...
        // 检查排序主键
        for (CkColumnVO ckColumn : buildTableInfoVo.getCkOrderByColumn()) {
            if (StringUtils.isBlank(ckColumn.getColumnName())) {
                throw SystemException.asSystemException(ErrorCodeEnum.CHECK_ERROR, "排序主键的字段不能为空");
            }
            if (!columnList.contains(ckColumn.getColumnName().toUpperCase())) {
                throw SystemException.asSystemException(ErrorCodeEnum.CHECK_ERROR, String.format("排序主键的字段[%s]不在建表字段中", ckColumn.getColumnName()));
            }
        }
        // 检查分区主键
        if (buildTableInfoVo.getCkPartitionColumn() != null) {
            for (CkColumnVO ckColumn : buildTableInfoVo.getCkPartitionColumn()) {
                if (StringUtils.isBlank(ckColumn.getColumnName())) {
                    throw SystemException.asSystemException(ErrorCodeEnum.CHECK_ERROR, "分区主键的字段不能为空");
                }
                if (!columnList.contains(ckColumn.getColumnName().toUpperCase())) {
                    throw SystemException.asSystemException(ErrorCodeEnum.CHECK_ERROR, String.format("分区主键的字段[%s]不在建表字段中", ckColumn.getColumnName()));
                }
            }
        }
        String createSql = this.getCreateSql(buildTableInfoVo);
